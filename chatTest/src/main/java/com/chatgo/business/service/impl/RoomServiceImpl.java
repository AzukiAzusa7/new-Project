...
public class RoomServiceImpl implements RoomService {
	...
	//チャット機能のみの場合
	public Room ifUserLogin(@AuthenticationPrincipal LoginUserDetails loginUserDetails, Pageable pageable, Model model){
		if(loginUserDetails == null){
			return room = roomService.findAll().get(0);
		}else{
			return room = roomUserService.findOne(loginUserDetails.getUserId());
		}
	}
	//ルーム機能が追加された場合
